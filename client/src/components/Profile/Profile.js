            ...
            <div className="ProfilePictures">
              <Slider {...settings} className="Slider">
                ...
                {Object.keys(photo).map(
                  (value, id) => (photo[value] && photo[value].length !== 6 && photo.master !== value ? <Image key={`${photo[id]}-${idUser}`} className="Grabber ProfilePictures" src={`data:image/jpg;base64,${photo[value]}`} /> : null)
                )}
              </Slider>
            </div>
